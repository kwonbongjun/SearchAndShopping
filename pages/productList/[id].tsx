...
export default function ProductList() { 
    ...
    const products = Data.productList.filter(item => 
        item.category === router.query.id
    )
    const categoriesEl = 
    Data.categories.map(category => {
        return (
            <span 
                className={`${style.category} ${category.id === router.query.id? style['select-category']: ''}`}
                 key={category.name}
            >
                <a href={`/productList/${category.id}`}>{category.name}</a>
            </span>
        )
    })
